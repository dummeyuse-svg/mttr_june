...
OLLAMA_MODEL_PLANNER = "llama3.2:3b"   # planner / text-to-SQL ONLY
...
async def make_plan(query: str) -> dict:
    prompt = _PLANNER_PROMPT.format(
        ...
        value_hints=_planner_value_hints() or "  (none indexed yet)",
        query=query,
    )
    try:
        raw  = await ask_ollama(prompt, max_tokens=450, model=OLLAMA_MODEL_PLANNER)
        plan = _parse_plan_json(raw)
    except Exception as e:
        print(f"[Planner] failed: {e}")
        plan = None
    if plan is None:
        # Safe fallback: treat as a semantic question
        plan = {"mode": "semantic", "sql": "", "semantic_query": query,
...
def build_v2_answer_prompt(query: str, plan: dict, rows: list[dict],
    ...
    # Strip system/binary fields
    clean_rows = []
    for r in rows[:40]:
        clean = {k: v for k, v in r.items()
                 if k not in _SYSTEM_COLS
                 and k not in ("image_b64", "image_name", "image_mime")
                 and v not in (None, "", -1, -1.0)}
        if clean:
            clean_rows.append(clean)

    # Detect TSG vs MTTR rows and use the matching human-readable formatter
    is_tsg = any("issue" in r or "corrective" in r for r in clean_rows[:3])
    if not clean_rows:
        records_block = "(no records found)"
    elif is_tsg:
        records_block = format_tsg_records_for_prompt(clean_rows)
    else:
        records_block = format_records_for_prompt(clean_rows)

    return (f"""You are a senior SMD / industrial maintenance engineer answering a technician.

TECHNICIAN'S QUESTION: {query}
WHAT THEY WANT: {plan.get('answer_focus') or 'a direct answer'}
DATA SOURCE: {source_label}

━━━ DATA ({len(clean_rows)} records) ━━━
{records_block}
━━━ END ━━━
...
STRICT RULES:
- Use ONLY the data above. NEVER invent records, names, dates, or numbers.
- Answer EXACTLY what was asked.
- loss_time is in minutes (-1 means unknown).
- If data is empty, say no records found. You MAY add 2-3 lines labeled "From engineering knowledge:".
...
async def query_records_v2(req: QueryRequest):
    ...
    if visualization and not rows:
        ...
        if result_type != "records":
            ...
            ai_response = await ask_ollama(
                build_v2_answer_prompt(query, plan, rows, lang, source_label),
                max_tokens=900 if plan.get("output_format") == "table" else MAX_TOKENS,
            )
    else:
        ai_response = await ask_ollama(
            build_v2_answer_prompt(query, plan, rows, lang, source_label),
